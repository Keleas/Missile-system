...
// OUT=[];
// Arguments    : const struct0_T TG
//                struct1_T *MFR
//                double dT
//                struct2_T *MSG
// Return Type  : void
//
void MFR_V2_Cycle(const struct0_T TG, struct1_T *MFR, double dT, struct2_T *MSG)
{
  emxArray_real_T *Rsc;
  int i;
  int ibmat;
  ...
  for (i = 0; i < 3; i++) {
    for (i1 = 0; i1 < ibmat; i1++) {
      ...
      dRsc->data[i1 + dRsc->size[0] * i] = TG.R0->data[ntilerows + TG.R0->size[0]
        * i] + TG.V->data[ntilerows + TG.V->size[0] * i] * dT;
    }
  }
